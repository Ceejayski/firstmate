...
# Alternation, not a [❯›] bracket expression: under some grep implementations
# a bracket class of multi-byte UTF-8 characters is matched byte-wise, so any
# line merely sharing a leading byte (e.g. other box-drawing glyphs) would
