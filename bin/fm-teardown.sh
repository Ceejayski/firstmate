...
remove_qwen_turnend_auth() {
  local state_dir=$1 id=$2 token hooks_dir
  token=$(cat "$state_dir/$id.qwen-turnend-token" 2>/dev/null || true)
  case "$token" in ''|*[!A-Za-z0-9._-]*) return 0 ;; esac
  hooks_dir="${QWEN_HOME:-$HOME/.qwen}/fm-turn-end.d"
  rm -f "$hooks_dir/$token"
}
    ...
    if [ "$child_kind" = secondmate ]; then
      ...
      if [ -n "$child_wt" ] && [ -d "$child_wt" ]; then
        ...
        rm -f "$child_wt/.claude/settings.local.json" "$child_wt/.opencode/plugins/fm-turn-end.js" \
          ...
          "$child_wt/.fm-qwen-turnend" "$child_wt/.qwen/settings.json"
      fi
...
remove_qwen_turnend_auth "$STATE" "$ID"
...
rm -f "$STATE/$ID.status" "$STATE/$ID.turn-ended" "$STATE/$ID.meta" \
  ...
  "$STATE/$ID.kimi-turnend-token" "$STATE/$ID.qwen-turnend-token"
