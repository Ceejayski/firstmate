...
`fm-spawn` instead neutralizes the check for the task worktree only, by writing a `<worktree>/.qwen/settings.json` containing `{"providerMetadata": null}`.
Unlike the `.fm-*-turnend` pointers, that file is NOT added to `info/exclude`: `.qwen/settings.json` is qwen's ordinary workspace-config path, and `info/exclude` is clone-common and never cleaned up, so an entry there would permanently hide a later crewmate's own edits to that file from `git status` and `git add -A` in every worktree of the project.
It stays untracked instead, and `fm-teardown`'s dirty-check tolerance for `?? .qwen/` is what keeps it from refusing a ship task's worktree return.
qwen merges a workspace settings file over the user one, and the update check skips every provider whose merged metadata carries no version, so a null namespace disables the prompt without naming any provider, without changing which models exist (those come from `modelProviders`), and without touching a captain-owned file.
