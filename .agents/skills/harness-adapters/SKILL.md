...
| qwen | The `modelProviders` entries in `~/.qwen/settings.json` list only what this installation is CONFIGURED for. Configured is NOT the same as entitled: a full probe pass on 2026-08-07 measured 6 entitled and 10 denied ids in the same configured provider block, with the denials concentrated in the cheap and mid tiers - `deepseek-v4-flash` (`403 Access to model denied`), `deepseek-v3.2`, every Kimi id, `glm-5.1`, `glm-5`, `MiniMax-M2.5`, `qwen3.6-plus`, and `qwen2.6-flash` as `404 Model not exist`. Measure entitlement with `bin/fm-model-probe.sh`, which reads those ids and prints one entitled/denied/unknown line per id; its header owns the exact contract. Run it before a dispatch-routing edit that names a qwen model, never on a session-start or other automatic path. |
