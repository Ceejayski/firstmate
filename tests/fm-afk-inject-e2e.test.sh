...
submit_line() {
  ...
  # Match the MARK prefix with a quoted-literal case pattern rather than a
  # ${_line:0:1} substring: under a non-UTF-8 (e.g. C) locale, substring
  # extraction counts bytes, not the full multi-byte MARK character, so it
  # would never match.
  case "$_line" in
    "$MARK"*) _c="injection" ;;
    *) _c="user" ;;
