...
config/buzz-bridge.env  optional Buzz fleet-bridge settings; LOCAL, gitignored; a configured channel turns on the explicit-invocation status/research bridge (docs/buzz-bridge.md)
data/                personal fleet records; LOCAL, gitignored as a whole
  backlog.md         task queue, dependencies, history
  captain.md         this home's domain-local captain preferences and working style; LOCAL, gitignored, canonical even if harness memory mirrors it, and updated with inspect-then-update
  captain-shared.md  main-authoritative shared captain preferences propagated read-only to secondmate homes; LOCAL, gitignored, owned by secondmate-provisioning
  fable-reasoning-protocol.md  reasoning discipline read by crewmates and scouts; LOCAL, gitignored; referenced by every ship/scout brief's Reasoning (Fable protocol) section (section 4)
...
state/               volatile runtime signals; gitignored
  ...
  buzz-bridge.cursor  last-seen Buzz research-request timestamp for bin/fm-buzz-enqueue.sh; present only when the Buzz bridge is opted in (docs/buzz-bridge.md)
...
5. **Fleet-state digest** - the compact backlog listing owned by `bin/fm-session-start.sh`; every `state/<id>.meta`; a bounded tail of each task's `state/<id>.status` (labeled as wake-EVENT history, not current state, with the full log path printed for a deeper read); the `state/.afk` flag; one cheap alive/dead read of each task's recorded backend endpoint; and a per-task `quota:` line from `bin/fm-limit-sense.sh`.
   That liveness line is a fast presence check only, not a full state read - when you need a crew's actual current state (a run-step, not just "is the pane there"), read it with `bin/fm-crew-state.sh <id>` as before; the digest deliberately skips that deeper, slower read for every task so it stays fast and bounded.
   A provider limit death appends no status line, fires no turn-end hook, and leaves an endpoint that reads alive, so the `quota:` line is the only place it is visible: treat `limit: dead` whose `reset=` has already passed as recoverable idle time and act on it, and `limit: unknown` as no information rather than health.
...
The verified harnesses are `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, `kimi`, and `qwen`; never dispatch on an unverified adapter.
Crewmate and scout briefs from `bin/fm-brief.sh` include a Reasoning (Fable protocol) section referencing `data/fable-reasoning-protocol.md` (scaffolded for every harness at brief time; primary discipline for Grok).
...
Every ship `done:` line also carries the worker's `bin/fm-honest-done.sh --solo` branch-versus-target suite measurement, which `bin/fm-brief.sh` requires and that script's header defines; a missing, `unknown`, `UNVERIFIED`, or `CONTENDED` measurement is not a green suite and not a comparison verdict.
...
Every ship and scout brief must prohibit `git stash` because linked worktrees share the clone-wide `refs/stash`, and direct workers to scratch commits or temporary branches instead.
