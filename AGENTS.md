...
Every ship `done:` line also carries the worker's `bin/fm-honest-done.sh` branch-versus-target suite measurement, which `bin/fm-brief.sh` requires and `bin/fm-honest-done.sh`'s header defines; a missing or `unknown` measurement is not a green suite.
