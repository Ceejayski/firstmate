...
- **Optional Buzz fleet bridge** - opt in with a configured channel so firstmate can post public-safe fleet snapshots into a [Buzz](https://github.com/block/buzz) community and pull `fm research:` requests back out for firstmate-side dispatch, with zero behavior change until a channel is set.
...
- `skills/` - public, installer-facing skills meant to be installed standalone into any project, independent of firstmate.
  ...
  Today that is `skills/stow`, a generic session-knowledge-sweep skill that routes findings by explicit instruction first, then existing local conventions, then a private `.stow-notes.md` fallback in the current directory, and closes with a resume pointer for the next session; and `skills/showcase`, an artistic-direction skill that forces a cited DNA read, a committed thesis over three verified-distinct directions, a per-surface showcase test, one flagged surprise, and a subtractive pass, with field-test evidence in [docs/verification/showcase-skill.md](docs/verification/showcase-skill.md).
...
- [docs/buzz-bridge.md](docs/buzz-bridge.md) - the opt-in Buzz fleet bridge: status posting, research-request intake, and the dual-home Grok-research convention.
