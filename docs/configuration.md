...
FM_BACKEND_HERDR_BARE_PROMPT_RE='^(❯|›)'  # herdr-only: verified agent glyphs recognized as an UNBORDERED (bare) composer row, e.g. claude's ❯ or codex's ›; alternation rather than a bracket class so a non-UTF-8 locale can't byte-wise false-positive on another glyph sharing a leading byte; shell glyphs remain unknown rather than empty, and de-emphasised ghost/placeholder text (dim or dark-truecolor) after an agent prompt reads empty via the shared fm_composer_strip_ghost (docs/herdr-backend.md "Composer and injection safety")
...
FM_SESSION_START_CUSTODY_BUDGET=6   # limit-dead tasks per digest that also get the read-only stranded-validation check; beyond it the digest prints how many were deferred instead of truncating silently
...
FM_CREW_STATE_LIMIT_SENSE=1   # set to 0 to drop the limit-dead annotation fm-crew-state.sh appends from fm-limit-sense.sh
FM_LIMIT_SENSE_NM_TIMEOUT=10   # seconds allowed for the read-only `no-mistakes axi sync --check` stranded-validation query inside fm-limit-sense.sh
FM_CLAUDE_PROJECTS_DIR=   # alternate Claude Code transcript root read by fm-limit-sense.sh, mainly for tests; unset uses ${CLAUDE_CONFIG_DIR:-~/.claude}/projects
...
FM_CAPTAIN_RE='done:|needs-decision:|blocked:|failed:|ready-for-review:|ready-for-merge:|verdict:|returned:|review-dispatched:|PR ready|checks green|ready in branch|merged'   # captain-relevant status regex; the auto-pipeline verbs (ready-for-review:, ready-for-merge:, verdict:, returned:, review-dispatched:) wake firstmate but stay nonterminal, and nonterminal progress verbs remain excluded even when their prose matches
...
QWEN_HOME=              # optional Qwen config home for firstmate's qwen turn-end extension and its private token registry, and the settings directory fm-model-probe.sh reads model ids from; defaults to ~/.qwen
...
FM_MODEL_PROBE_SETTINGS=    # absolute path to the settings file fm-model-probe.sh reads (wins over QWEN_HOME); same as its --settings flag. Read-only: the probe never writes this file
FM_MODEL_PROBE_TIMEOUT=100  # per-model wall-clock seconds for one entitlement probe; same as fm-model-probe.sh --timeout
FM_MODEL_PROBE_QWEN=qwen    # qwen executable path or name fm-model-probe.sh probes with; a missing CLI reports unknown rather than failing
FM_MODEL_PROBE_WORKDIR=     # directory each probe call runs in; unset uses a fresh temp dir under ${TMPDIR:-/tmp}, removed on exit
