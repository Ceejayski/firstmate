...
| `fm-brief.sh`            | Scaffold task and secondmate briefs                                                   |
| `fm-context-pack.sh`     | Resolve a budgeted task context pack from the active home's fleet graph              |
| `fm-honest-done.sh`      | Compare project suite on branch vs merge target; one-line done-report measurement    |
...
| `fm-limit-sense.sh`      | Print one line naming a provider limit death read from a task's session transcript   |
| `fm-model-probe.sh`      | On-demand only: print one entitled/denied/unknown line per configured Qwen model id  |
