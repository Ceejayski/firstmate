...
- Only shared material is tracked: `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, `.agents/skills/`, and `skills/`.
  ...
  That standalone contract is enforced: `tests/fm-captain-translation-contract.test.sh` fails any `skills/*/SKILL.md` that names the private `.agents/` tree, a `bin/fm-*` helper, `FM_HOME`-relative state, a private project path, or firstmate house vocabulary such as captain or crewmate.
